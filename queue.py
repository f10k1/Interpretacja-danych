...
class interpretationOfQueue:
    ...
    def add(self):
        if (len(self.lista)<10):
            ...
            self.wypisz()
            self.canvas.create_text(80, 170, text='Kolejka jest pełna', fill='red')
    ...
    def usun(self):
        if len(self.lista) > 0:
            ...
            self.wypisz()
            self.canvas.create_text(80, 170, text='Kolejka jest pusta', fill='red')
    ...
    def pobierz(self):
        if len(self.lista)>0:
            self.wypisz()
            self.canvas.create_text(80, 420, text='Pobrano: '+self.lista[0], fill='green')
        else:
            self.wypisz()
            self.canvas.create_text(80, 420, text='Kolejka jest pusta', fill='red')

    def pobierz_usun(self):
        if len(self.lista)>0:
            item = 'Pobrano i usunięto: '+self.lista.pop(0)
            self.wypisz()
    ...
    def wypisz(self):
        ...
        dodaj = Button(self.master, text="Dodaj do kolejki", command=self.add, anchor="center")
        dodaj.configure(width=15, activebackground="lightgrey")

        self.canvas.create_window(40, 75, anchor=NW, window=dodaj)
        usun = Button(self.master, text="Usuń z kolejki", command=self.usun, anchor='center')
        usun.configure(width=15,activebackground = 'lightgrey')
        self.canvas.create_window(40, 120, anchor=NW, window=usun)

        pobierz = Button(self.master, text='Pobierz z kolejki', command=self.pobierz, anchor='center')
        pobierz.configure(width=15, activebackground='lightgrey')
        self.canvas.create_window(40, 320, anchor=NW, window=pobierz)

        pobierz_usun = Button(self.master, text='Pobierz i usuń z kolejki', command=self.pobierz_usun, anchor='center')
